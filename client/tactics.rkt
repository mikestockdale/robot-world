...
(define (choose-take bot block)
  (let ([take-direction (direction-from (bot-location bot) (occupant-location block))]) 
    (choice request-take (occupant-id block) take-direction)))
...
(define (choose-transfer bot base)
  (choice request-transfer (occupant-id base)
          (direction-from (occupant-location base) (bot-location bot))))
